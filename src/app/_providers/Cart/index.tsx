    ...
    const newTotal =
      cart?.items?.reduce((acc, item) => {
        ...
        if (typeof item?.product === 'object') {
          const priceJSON = item?.product?.priceJSON

          if (typeof priceJSON === 'string') {
            // If priceJSON is a number
            priceValue = Number(priceJSON)
          } else {
            try {
              // If priceJSON is a string, parse it
              const parsedPrice = JSON.parse(priceJSON || '{}')?.data?.[0]?.unit_amount
